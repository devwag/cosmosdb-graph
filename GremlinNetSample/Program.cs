...
namespace GremlinNetSample
{
    ...
    class Program
    {
        ...
        private static string hostname = "<yourgraph>.gremlin.cosmos.azure.com";
        private static int port = 443;
        private static string authKey = "<yourkey>";
        private static string database = "sample-database";
        private static string collection = "sample-graph";
        ...
        private static Dictionary<string, string> gremlinQueries = new Dictionary<string, string>
        {
            ...
            { "AddVertex 3",    "g.addV('person').property('id', 'benji').property('firstName', 'Ben').property('lastName', 'Miller').property('pk', 'pk')" },
            { "AddVertex 4",    "g.addV('person').property('id', 'robin').property('firstName', 'Robin').property('lastName', 'Wakefield').property('pk', 'pk')" },
            { "AddEdge 1",      "g.V('thomas').addE('knows').to(g.V('mary'))" },
            { "AddEdge 2",      "g.V('thomas').addE('knows').to(g.V('benji'))" },
            { "AddEdge 3",      "g.V('benji').addE('knows').to(g.V('robin'))" },
